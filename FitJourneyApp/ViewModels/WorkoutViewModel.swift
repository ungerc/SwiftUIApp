...
@MainActor
class WorkoutViewModel {
    private let workoutAdapter: WorkoutAdapter
    
    init(workoutAdapter: WorkoutAdapter) {
    ...
    func fetchWorkouts() async {
        ...
        do {
            let fetchedWorkouts = try await workoutAdapter.fetchWorkouts()
            workouts = fetchedWorkouts
            isLoading = false
        } catch {
            errorMessage = "Failed to fetch workouts: \(error.localizedDescription)"
    ...
    func addWorkout(_ workout: Workout) async {
        ...
        do {
            let newWorkout = try await workoutAdapter.addWorkout(workout)
            workouts.append(newWorkout)
            workouts.sort { $0.date > $1.date }
            isLoading = false
        } catch {
